...
CTEST(rbtree_test, rb_find_test) {
  ...
  for (const uintptr_t *it = testcases; it < testcases + sizeof(testcases)/sizeof(uintptr_t); ++it)
    ASSERT_EQUAL_U(*it, (uintptr_t)rb_find(tree, (void *)*it));
...
CTEST(rbtree_test, rb_insert_test) {
  ...
  for (const uintptr_t *it = testcases; it < testcases + sizeof(testcases)/sizeof(uintptr_t); ++it)
    ASSERT_NOT_NULL(rb_insert(&tree, (void *)*it, NULL));

  memset(dest, 0, sizeof(dest));
  rb_for_each(tree, concat);
...
CTEST(rbtree_test, rb_insert_or_assign_test) {
  ...
  for (const uintptr_t *it = testcases; it < testcases + sizeof(testcases)/sizeof(uintptr_t); ++it)
    ASSERT_NOT_NULL(rb_insert_or_assign(&tree, (void *)*it, NULL));

  for (const uintptr_t *it = testcases; it < testcases + sizeof(testcases)/sizeof(uintptr_t); ++it)
    ASSERT_NOT_NULL(rb_insert_or_assign(&tree, (void *)*it, (void *)*it));
